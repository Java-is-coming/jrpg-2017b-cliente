...
public class Entidad {
    ...
    public void graficarNombre(final Graphics g) {
        ...
        String trucos = "<";
        if (getJuego().getPersonaje().getModoDios()) {
            trucos += " GOD ";
        }
        ...
        if (trucos != "<") {
            trucos += ">";
            final int offsetY2 = 6;
            final int fontSize2 = 12;
            final int red = 153;
            final int greenblue = 255;
            g.setColor(new Color(red, greenblue, greenblue));
            g.setFont(new Font("Book Antiqua", Font.BOLD, fontSize2));
            Pantalla.centerString(g, new java.awt.Rectangle(drawX + offsetx, drawY - offsetY2, 0, height), trucos);
        }
    }
